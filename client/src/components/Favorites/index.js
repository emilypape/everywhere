...
function Favorites() {
    ...
    return (
        <div className='I-M-Here'>
            <h2>ID's!</h2>
            {favorites && favorites[0].map((favorite) => (
               <p key={favorite.favoriteId}>
                favoriteID: {favorite.favoriteId}<br />
                favorite Title: {favorite.favoriteTitle}<br />
                favorite image: {favorite.favoritePhoto}<br />
                favorite price: {favorite.favoritePrice}<br /><br />
                </p>
            ))}
        </div> 
    )
